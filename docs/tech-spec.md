...
## 11. Auth — единый вход
- Логин/регистрация/refresh/logout/change-password лежат в `/auth/*`.
- Access token хранится в памяти, refresh token — в httpOnly cookie.
- Роли: client (саморегистрация), partner/courier/admin/support — только через админку.

## 12. Партнёры: реквизиты, верификация, комиссия
- Партнёр заполняет реквизиты в статусах draft/rejected и отправляет на проверку.
- Админ подтверждает или отклоняет реквизиты (с комментарием при reject).
- Индивидуальная комиссия партнёра хранится в `commission_percent` и применяется к сумме блюд.
- Параметр `payout_hold` блокирует выплаты (учёт для финансовых операций).
- Связь пользователей и партнёров хранится в `partner_users`.
- Точки партнёра хранятся в `points` (адрес, телефон, режим работы, статус).

## 13. Код передачи заказа (handoff)
- Для каждого заказа генерируется код передачи (4–6 цифр).
- В базе хранится только хэш и зашифрованная версия (для выдачи по правам).
- Доступ к коду зависит от роли и статуса заказа.
- Подтверждение передачи выполняется через `/api/orders/:id/confirm-handoff`.

## 14. Меню точек партнёра (miniapp)
- Партнёр управляет меню по точкам: категории и позиции.
- Категории уникальны в рамках точки (name_normalized).
